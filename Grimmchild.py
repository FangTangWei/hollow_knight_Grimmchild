...
from PyQt5.QtWidgets import QApplication, QWidget, QSystemTrayIcon, QMenu, QAction, QActionGroup, QInputDialog
...
def get_int_input(title, label, value, min_val, max_val, step=1):
    ...
    d.setWindowTitle(title); d.setLabelText(label); d.setIntValue(value)
    d.setIntMinimum(min_val); d.setIntMaximum(max_val); d.setIntStep(step)
    return (d.intValue(), True) if d.exec_() == QInputDialog.Accepted else (0, False)

ANIM_FRAMES = {}; audio_manager = None

def load_animations(base_dir):
    if not os.path.exists(base_dir): raise FileNotFoundError(f"目录不存在: {base_dir}")
    names = {'flameball impact': 'flameball_impact', 'flameball': 'flameball', 'burst': 'burst',
    ...
    for folder in os.listdir(base_dir):
        ...
        if not os.path.isdir(fp) or '.' not in folder: continue
        raw = folder.split('.', 1)[1].strip().lower()
        for k, base in names.items():
            if raw.startswith(k):
                rest = raw[len(k):].strip()
                stage = int(rest) if rest.isdigit() else None
                break
        else: continue
        frames = sorted((int(f.split('-')[1]), f) for f in os.listdir(fp)
                        if f.lower().endswith('.png') and '-' in f and f.split('-')[1].isdigit())
        if frames:
            ANIM_FRAMES[(base, stage)] = [QPixmap(os.path.join(fp, f)) for _, f in frames]

class AudioManager:
    def __init__(self, audio_dir):
        self.audio_dir = audio_dir; self.audio_enabled = True
        self.bgm_player = None; self.current_bgm = None; self.bgm_volume = 100; self._players = []
        self.sounds = {k: os.path.join(audio_dir, v) for k, v in {
            'attack_yelp_1_3': '1Grimmbat_attack_yelp_1~3.mp3', 'attack_yelp_4': '1Grimmbat_attack_yelp_4.mp3',
            'fireball_yelp_1_3': '2Grimmbat_attack_yelp_1~3.mp3', 'fireball_yelp_4': '2Grimmbat_attack_yelp_4.mp3',
            'teleport_out': 'grimm_teleport_out.mp3', 'idle_1_3': 'Grimmbat_idle_1~3.mp3', 'idle_4': 'Grimmbat_idle_4.mp3',
            'fly_loop': 'grimmchild_fly_loop.mp3', 'fireball_shoot': 'grimmchild_fireball_shoot.mp3',
            'bgm_grimm': 'Grimm.mp3', 'bgm_epic': 'Grimm Epic Layer.mp3'}.items()
...
class Fireball:
    def __init__(self, start_pos, target_pos, owner=None, speed=12):
        self.pos = QPointF(start_pos); self.owner = owner
        dx, dy = target_pos.x() - start_pos.x(), target_pos.y() - start_pos.y()
        dist = math.hypot(dx, dy) or 1
        self.vx, self.vy = dx / dist * speed, dy / dist * speed
        self.active = True; self.impacting = False; self.frame_idx = 0; self.timer = 0
        self.frames = ANIM_FRAMES.get(('flameball', None), []); self.impact_frames = ANIM_FRAMES.get(('flameball_impact', None), [])
    ...
    def update(self):
        if self.impacting:
            ...
            return
        x, y = self.pos.x() + self.vx, self.pos.y() + self.vy
        self.pos.setX(x); self.pos.setY(y)
        self.timer += 1
        if self.frames and self.timer % 3 == 0: self.frame_idx = (self.frame_idx + 1) % len(self.frames)
        if not (-100 <= x <= self.screen_rect.width() + 100 and -100 <= y <= self.screen_rect.height() + 100):
            self.active = False

    def hit_check(self, pet):
        if not self.active or self.impacting or pet is self.owner: return False
        if abs(pet.x - self.pos.x()) < 60 and abs(pet.y - self.pos.y()) < 60:
...
class Pet:
    ...
    _ANIM_NAMES = {FLY: 'fly', IDLE: 'idle', TURN_TO_IDLE: 'turntoidle', TELE_OUT: 'tele_out',
                   TELE_IN: 'tele_in', SLEEP_ANIM: 'sleep', SLEEP_STILL: 'sleep', WAKE: 'wake',
                   ANTIC: 'antic', SHOOT: 'shoot', BURST: 'burst'}
    ...
    def _update_anim(self):
        base = Pet._ANIM_NAMES.get(self.state)
        if base is None: return
        key = (base, None if base == 'burst' else self.stage)
        frames = ANIM_FRAMES.get(key)
        if frames is None and key[1] is not None:
            for s in range(1, 5):
                if (key[0], s) in ANIM_FRAMES: frames = ANIM_FRAMES[(key[0], s)]; break
        if frames:
    ...
    def _start_attack(self, target):
        ...
        if r < 0.4:
            ...
            dx, dy = target.x - self.x, target.y - self.y; dist = math.hypot(dx, dy) or 1
            self.vx, self.vy = dx / dist * 3.0, dy / dist * 3.0
            self.state = self.FLY; self._update_anim(); self.attack_move_distance = 0
        else:
            self.attack_mode = 'post_teleport' if r < 0.6 else 'teleport_far'
            radius = 80 + random.uniform(0, 40) if r < 0.6 else 500 + random.uniform(0, 200)
            ...
            dx, dy = target.x - tx, target.y - ty; dist = math.hypot(dx, dy) or 1
            self.vx, self.vy = dx / dist * 3.0, dy / dist * 3.0
    ...
    def hit(self, damage, attacker=None):
        ...
        if self.hp <= 0:
            ...
            self._die(); return
        if attacker:
            if self.state in (self.SLEEP_ANIM, self.SLEEP_STILL): self.hovering = False; self.change_state(self.WAKE)
            if self.can_attack(): self.attack_target = attacker; self.attack_cooldown = 0

    def on_kill(self):
        if self.stage < 4: self.stage += 1
        self._apply_owner_hp(); self.attack_mode = None; self.attack_target = None; self.no_target_attack = False
        self._stop_fly_loop(); self.change_state(self.TELE_OUT)
    ...
    def update(self):
        ...
        if self.attack_mode in ('fly_move', 'post_teleport', 'teleport_far') and self.state is self.FLY:
            ...
            if self.attack_mode == 'fly_move':
                ...
                done = self.attack_move_distance >= self.attack_move_required
            else:
                self.post_teleport_timer -= 1
                done = self.post_teleport_timer <= 0
            if done:
        ...
        if self.hovering:
            ...
            if self.state is self.FLY:
                ...
                if self.no_target_attack:
                    tgt = QPointF(self.x + 200 * self.direction, self.y)
                    self.no_target_attack = False; self.attack_target = None
                elif self._target_alive():
                    tgt = QPointF(self.attack_target.x, self.attack_target.y)
                else: self._on_target_lost(); return
                self.owner.fireballs.append(Fireball(QPointF(self.x, self.y), tgt, owner=self,
                                                     speed=self.owner.fireball_speed))
...
class DeskPetWindow(QWidget):
    ...
    def _random_missing_stage(self):
        missing = [s for s in range(1, 5) if s not in {p.stage for p in self.pets}]
    ...
    def game_loop(self):
        ...
        for pet in self.pets:
            ...
            if pet.pending_removal and pet.state is pet.TELE_OUT and pet.anim_finished:
                pet._stop_fly_loop(); gone.append(pet)
        for p in gone: self.pets.remove(p)
        ...
        for fb in gone: self.fireballs.remove(fb)
    ...
    def handle_split(self):
        ...
        if self.split_timer <= 0:
            ...
            new_pet = self._new_pet(self._random_missing_stage(),
                                    source.x + random.randint(-60, 60), source.y + random.randint(-60, 60))
    ...
    def show_context_menu(self, pet, pos):
        ...
        for i in range(1, 5):
            a = QAction(f"阶段 {i}", stage_menu, checkable=True)
            if i == pet.stage: a.setChecked(True)
            a.triggered.connect(lambda checked, s=i: self.set_pet_stage(pet, s))
            stage_group.addAction(a); stage_menu.addAction(a)
        for text, fn in [("休眠", lambda: self.sleep_pet(pet)), ("唤醒", lambda: self.wake_pet(pet)),
                         ("分裂", lambda: self.split_pet(pet)), ("瞬移", lambda: self.teleport_pet(pet))]:
            menu.addAction(text, fn)
        if pet.can_attack():
            menu.addAction("攻击", lambda: self.attack_from_menu(pet))
            menu.addAction("大乱斗", lambda: self.battle_royale_pet(pet))
        ...
        for text, enable in [("是", True), ("否", False)]:
            a = QAction(text, peace_menu, checkable=True)
            if self.peace_mode == enable: a.setChecked(True)
            a.triggered.connect(lambda checked, e=enable: self.set_peace_mode(e))
            peace_group.addAction(a); peace_menu.addAction(a)
        menu.addSeparator()
        bgm = menu.addMenu("背景音乐")
        bgm_group = QActionGroup(bgm)
        for key, label in {'bgm_grimm': 'Grimm', 'bgm_epic': 'Grimm Epic Layer'}.items():
            a = QAction(label, bgm, checkable=True)
            if audio_manager and audio_manager.current_bgm == key: a.setChecked(True)
            a.triggered.connect(lambda checked, k=key: self.set_bgm(k))
            bgm_group.addAction(a); bgm.addAction(a)
        bgm.addSeparator()
    ...
    def split_pet(self, pet):
        ...
        new_pet = self._new_pet(self._random_missing_stage(),
                                pet.x + random.randint(-60, 60), pet.y + random.randint(-60, 60))
    ...
    def set_peace_mode(self, enable):
        ...
        if enable:
            for p in self.pets:
                p.battle_royale = False; p.attack_target = None; p.attack_mode = None
                p.attack_cooldown = 0; p.no_target_attack = False
            self.battle_royale_global = False
    ...
    def paintEvent(self, event):
        ...
        for pet in self.pets:
            ...
            if frame:
                if pet.direction == -1:
                    flipped = self._flip_cache.get(id(frame))
                    if flipped is None:
                        flipped = frame.transformed(DeskPetWindow.FLIP_X)
                        self._flip_cache[id(frame)] = flipped
                    frame = flipped
...
if __name__ == '__main__':
    ...
    try:
        load_animations(os.path.join(base, 'Grimmchild Anim'))
    except FileNotFoundError as e:
        print(e); sys.exit(1)
    audio_manager = AudioManager(os.path.join(base, 'AudioClip'))

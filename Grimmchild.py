...
class AudioManager:
    def __init__(self, audio_dir):
        ...
        self.sounds = {k: os.path.join(audio_dir, v) for k, v in sounds_map.items() if os.path.exists(os.path.join(audio_dir, v))}
    ...
    def create_loop_player(self, key):
        ...
        player = QMediaPlayer()
        player.setMedia(QMediaContent(QUrl.fromLocalFile(self.sounds[key])))
        player.mediaStatusChanged.connect(lambda s, p=player: (p.setPosition(0), p.play()) if s == QMediaPlayer.EndOfMedia else None)
        return player
    ...
    def play_bgm(self, key):
        ...
        if key in self.sounds:
            ...
            self.bgm_player.mediaStatusChanged.connect(lambda s: (self.bgm_player.setPosition(0), self.bgm_player.play()) if s == QMediaPlayer.EndOfMedia else None)
            if self.audio_enabled: self.bgm_player.play()
            self.current_bgm = key

    def stop_bgm(self):
        if self.bgm_player: self.bgm_player.stop(); self.bgm_player.deleteLater(); self.bgm_player = None; self.current_bgm = None
...
class Fireball:
    def __init__(self, start_pos, target_pos, owner=None, speed=12):
        ...
        dx = target_pos.x() - start_pos.x(); dy = target_pos.y() - start_pos.y()
    ...
    def hit_check(self, pet):
        ...
        if pet_rect.intersects(fire_rect):
            self.impacting = True; self.frame_idx = 0; self.timer = 0
            return True
        return False

    def current_frame(self):
        if self.impacting and self.impact_frames and self.frame_idx < len(self.impact_frames): return self.impact_frames[self.frame_idx]
...
class Pet:
    ...
    def __init__(self, stage=1):
        ...
        self._turn_pending = False; self._pending_turn_dir = None; self._turn_speed_x = 0; self._turn_vy = 0
        self._update_anim()

    def _random_motion(self):
        self.vx = random.choice([-1.5, 1.5]); self.vy = random.choice([-0.8, 0.8])
    ...
    def _stop_fly_loop(self):
        if self.fly_loop_player: self.fly_loop_player.stop(); self.fly_loop_player.deleteLater(); self.fly_loop_player = None
    ...
    def change_state(self, new_state):
        ...
        self._update_anim(); self._trigger_state_sound()

    def _target_alive(self):
        return self.attack_target and not self.attack_target.pending_removal and self.attack_target in self.owner.pets
    ...
    def _start_attack(self, target):
        ...
        if r < 0.5:
            ...
            angle = random.uniform(0, 2 * math.pi); radius = 80 + random.uniform(0, 40)
            target_x = target.x + math.cos(angle) * radius; target_y = target.y + math.sin(angle) * radius
            rect = self.owner.screen_rect
            target_x = max(80, min(rect.width() - 80, target_x)); target_y = max(80, min(rect.height() - 80, target_y))
            self._teleport_target_x = target_x; self._teleport_target_y = target_y
            dx, dy = target.x - target_x, target.y - target_y
            dist = math.hypot(dx, dy) or 1
            self.vx, self.vy = (dx / dist) * 3.0, (dy / dist) * 3.0
            self.attack_mode = 'post_teleport'; self.post_teleport_timer = 6
    ...
    def hit(self, damage, attacker=None):
        ...
        self.hp -= damage
        if attacker:
            self.pending_direction = 1 if attacker.x >= self.x else -1

        if self.hp <= 0:
            if attacker and attacker.hp > 0 and attacker.can_attack():
                attacker.on_kill()
            self._die()
            return
    ...
    def on_kill(self):
        if self.stage < 4: self.stage += 1; self.max_hp = self.stage
        self.hp = self.max_hp; self.attack_mode = None; self.attack_target = None
    ...
    def _move_random(self):
        ...
        if self.x < margin:
            ...
            if self.direction == -1 and self.vx < 0 and not self._turn_pending and self.state is not self.TURN_TO_IDLE:
                self._turn_pending = True; self._pending_turn_dir = 1; self._turn_speed_x = abs(self.vx); self._turn_vy = self.vy
    ...
    def update(self):
        ...
        if self.attack_mode == 'fly_move':
            old_x, old_y = self.x, self.y; self.x += self.vx; self.y += self.vy
            ...
            return
        if self.attack_mode == 'post_teleport' and self.state is self.FLY:
            self.post_teleport_timer -= 1; self.x += self.vx; self.y += self.vy
            if not self._target_alive(): return self._on_target_lost()
            if self.post_teleport_timer <= 0: self.attack_mode = None; self.vx = self.vy = 0; self.change_state(self.ANTIC); return
            self._advance_anim(4)
            if self.vx != 0: self.direction = 1 if self.vx >= 0 else -1
            return
        ...
        if self.hovering:
            self.vx = self.vy = 0
            if self.state not in (self.IDLE, self.TURN_TO_IDLE, self.SLEEP_ANIM, self.SLEEP_STILL): self.change_state(self.IDLE)
        else:
            self._update_timers()
            if (self.attack_target and self.attack_cooldown <= 0 and self.state in (self.FLY, self.IDLE) and self.can_attack()):
                if self._target_alive(): self._start_attack(self.attack_target)

            if self.state is self.FLY:
                if self._turn_pending:
                    self._turn_pending = False; self.vx = self.vy = 0; self.change_state(self.TURN_TO_IDLE); return
            ...
            elif self.state is self.TURN_TO_IDLE:
                if self.anim_finished:
                    if self._pending_turn_dir is not None:
                        self.direction = self._pending_turn_dir
                        self.vx = self._turn_speed_x * (1 if self.direction == 1 else -1); self.vy = self._turn_vy
                        rect = self.owner.screen_rect; margin = 80
                        if self.x < margin + 5: self.x = margin + 5
                        elif self.x > rect.width() - margin - 5: self.x = rect.width() - margin - 5
            ...
            elif self.state is self.TELE_OUT:
                if self.anim_finished:
                    if self._teleport_target_x is not None:
                        ...
                        rect = self.owner.screen_rect
                        self.x, self.y = random.randint(150, rect.width() - 150), random.randint(150, rect.height() - 150)
                    self.change_state(self.TELE_IN)
            elif self.state is self.TELE_IN:
                if self.anim_finished:
                    if self._sleep_after_teleport:
                        self._sleep_after_teleport = None; self.change_state(self.SLEEP_ANIM)
                    elif self.attack_mode == 'post_teleport': self.state = self.FLY; self._update_anim()
                    else: self._random_motion(); self.change_state(self.FLY)
            elif self.state is self.SLEEP_ANIM:
                if self.anim_finished: self.change_state(self.SLEEP_STILL); self.sleep_duration = random.randint(600, 3600)
            elif self.state is self.SLEEP_STILL:
                if not self.hovering:
                    self.sleep_duration -= 1
                    if self.sleep_duration <= 0: self.change_state(self.WAKE)
            elif self.state is self.WAKE:
                if self.anim_finished:
                ...
                if self.anim_finished:
                    if self.no_target_attack:
                        ...
                        self.owner.fireballs.append(Fireball(QPointF(self.x, self.y), QPointF(self.x + dx, self.y), owner=self))
                        self.no_target_attack = False; self.attack_target = None; self._random_motion(); self.change_state(self.pre_attack_state)
                    elif self._target_alive():
                        target = self.attack_target
                        self.owner.fireballs.append(Fireball(QPointF(self.x, self.y), QPointF(target.x, target.y), owner=self))
                        self.attack_cooldown = 120; self.attack_target = target; self._random_motion(); self.change_state(self.pre_attack_state)
                    else: self._on_target_lost()
            elif self.state is self.BURST:
                if self.anim_finished:
                    if self.stage < 4: self.stage += 1; self.max_hp = self.stage; self.hp = self.max_hp
                    self.growth_timer = random.randint(3750, 11250)   # 升级后重置成长时间
                    self._random_motion(); self.change_state(self.FLY)

        self._advance_anim(3 if self.state in (self.TELE_OUT, self.TELE_IN) else 4)
        if self.pending_direction is not None: self.direction = self.pending_direction; self.pending_direction = None
        elif self.state is self.FLY and not self.hovering and self.vx != 0: self.direction = 1 if self.vx >= 0 else -1
    ...
    def _advance_anim(self, delay):
        ...
        if self.anim_timer >= delay:
            ...
            if self.anim_loop: self.anim_idx = (self.anim_idx + 1) % len(self.anim_frames)
                ...
                if self.anim_idx < len(self.anim_frames) - 1: self.anim_idx += 1
                else: self.anim_finished = True

    def _is_stationary(self): return abs(self.vx) < 0.1 and abs(self.vy) < 0.1

    def _update_timers(self):
        # ========== 修改：删除了原来的 growth_timer 递减 ==========
        if self.state in (self.FLY, self.IDLE, self.TURN_TO_IDLE):
            # 瞬移计时器仍然只在自由活动时减少
        ...
        if self.state in (self.FLY, self.IDLE) and not self.hovering:
    ...
    def initiate_sleep(self):
        ...
        if random.random() < 0.5:
            ...
            target_x = max(100, min(rect.width() - 100, self.x)); target_y = rect.height() - 100
            self._teleport_target_x = target_x; self._teleport_target_y = target_y
...
class DeskPetWindow(QWidget):
    ...
    def __init__(self):
        ...
        first_pet = Pet(stage=1); first_pet.owner = self; self.pets.append(first_pet)
    ...
    def game_loop(self):
        for pet in self.pets:
            if pet.sleep_pending == 'fly_to_bottom' and pet.y >= pet.target_sleep_y:
                pet.y = pet.target_sleep_y; pet.vy = 0; pet.change_state(pet.SLEEP_ANIM); pet.sleep_pending = None
            ...
            if pet.pending_removal:
                ...
                for other in self.pets:
                    if other.attack_target is pet:
                        ...
                        if other.state in (other.ANTIC, other.SHOOT): other._random_motion(); other.change_state(other.FLY)
    ...
    def handle_split(self):
        ...
        if self.split_timer <= 0:
            ...
            source = random.choice(self.pets); new_stage = random.choice(missing)
            new_pet = Pet(stage=new_stage); new_pet.owner = self
            new_pet.x = source.x + random.randint(-60, 60); new_pet.y = source.y + random.randint(-60, 60)
            new_pet.state = new_pet.TELE_IN; new_pet._update_anim()
            self.pets.append(new_pet); source.attack_cooldown = max(source.attack_cooldown, 120)

    def handle_random_attack(self):
        if len(self.pets) < 2: return
        self.auto_attack_timer -= 1
        if self.auto_attack_timer <= 0:
            self.auto_attack_timer = 3600
            candidates = [p for p in self.pets if p.state in (p.FLY, p.IDLE) and p.attack_cooldown <= 0 and not p.hovering and p.can_attack()]
            if not candidates: return
            attacker = random.choice(candidates)
            targets = [p for p in self.pets if p is not attacker]
            if targets: attacker._start_attack(random.choice(targets)); attacker.attack_cooldown = 180
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.RightButton:
            ...
            if clicked:
                ...
                if clicked.state not in (clicked.IDLE, clicked.SLEEP_ANIM, clicked.SLEEP_STILL): clicked.change_state(clicked.IDLE)
            ...
            for pet in self.pets:
                if pet.hit_test(event.pos()):
                    self.dragging_pet = pet; self.drag_offset = QPointF(pet.x - event.pos().x(), pet.y - event.pos().y())
    ...
    def mouseMoveEvent(self, event):
        if self.dragging_pet:
            ...
            pet.x = event.pos().x() + self.drag_offset.x(); pet.y = event.pos().y() + self.drag_offset.y()
            rect = self.screen_rect; margin = 80
            pet.x = max(margin, min(rect.width() - margin, pet.x)); pet.y = max(margin, min(rect.height() - margin, pet.y))
            self.update()

    def mouseReleaseEvent(self, event):
        if event.button() == Qt.LeftButton and self.dragging_pet:
            pet = self.dragging_pet; pet.hovering = False; pet.change_state(pet.FLY); pet._random_motion()
    ...
    def show_context_menu(self, pet, pos):
        ...
        bgm_items = {'bgm_grimm': 'Grimm', 'bgm_epic': 'Grimm Epic Layer'}
        for key, label in bgm_items.items():
            action = QAction(label, bgm_menu, checkable=True)
            ...
            bgm_group.addAction(action); bgm_menu.addAction(action)
        bgm_menu.addSeparator()
        bgm_menu.addAction("音量调节", self.manual_set_bgm_volume)
        bgm_menu.addAction("关闭", self.stop_bgm)

        menu.addAction("停止音频" if (audio_manager and audio_manager.audio_enabled) else "开始音频", self.toggle_audio)
        menu.addSeparator()
    ...
    def set_pet_stage(self, pet, stage):
        if pet.stage != stage:
            pet.stage = stage; pet.max_hp = stage; pet.hp = stage; pet._update_anim()
    ...
    def split_pet(self, pet):
        ...
        missing = [s for s in range(1,5) if s not in used_stages]
        if not missing: return
        new_stage = random.choice(missing); new_pet = Pet(stage=new_stage); new_pet.owner = self
        new_pet.x = pet.x + random.randint(-60, 60); new_pet.y = pet.y + random.randint(-60, 60)
        new_pet.state = new_pet.TELE_IN; new_pet._update_anim()
    ...
    def manual_set_bgm_volume(self):
        ...
        vol, ok = QInputDialog.getInt(self, "音量调节", f"音量 (0-100):", value=audio_manager.bgm_volume, min=0, max=100, step=5)
    ...
    def paintEvent(self, event):
        ...
        for fb in self.fireballs:
            ...
            if frame: painter.drawPixmap(int(fb.pos.x() - frame.width()//2), int(fb.pos.y() - frame.height()//2), frame)
        for pet in self.pets:
            frame = pet.current_frame()
            if frame:
                if pet.direction == -1: frame = frame.transformed(DeskPetWindow.FLIP_X)
                painter.drawPixmap(int(pet.x - frame.width()//2), int(pet.y - frame.height()//2), frame)

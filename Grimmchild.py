            ...
            if self.state == 'fly':
                ...
                if self.anim_finished:
                    if self.no_target_attack:
                        ...
                        fb = Fireball(QPointF(self.x, self.y),
                                      QPointF(self.attack_target.x, self.attack_target.y),
                                      owner=self)
                        if self.owner:
                            self.owner.fireballs.append(fb)
                    self.attack_target = None
...
class DeskPetWindow(QWidget):
    ...
    def show_context_menu(self, pet, pos):
        ...
        # 音量调节子菜单
        vol_menu = bgm_menu.addMenu("音量调节")
        for vol, label in [(25, "25%"), (50, "50%"), (75, "75%"), (100, "100%")]:
            vol_action = QAction(label, vol_menu)
            vol_action.triggered.connect(lambda checked, v=vol: self.set_bgm_volume(v))
